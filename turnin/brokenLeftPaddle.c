...
int LP_Tick(int state) {
	unsigned char tmpD = ~(0x03 & PIND);
	// Actions
	switch (state) {
		case sampleLP:
			// update dot in horizental direction
			if(tmpD == 0x02){
				if(lPadY > 0) lPadY = lPadY - 1;
				else lPadY = 0x00; 
			}
			else if(tmpD == 0x01){
